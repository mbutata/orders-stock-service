...
| `errors` | `validation_error` and `unknown_sku` only | One entry per offending location: `detail`, plus exactly one of `pointer` (RFC 6901 JSON Pointer into the body, in URI fragment form such as `#/items/0/qty`) or `parameter` (a query or path parameter name); the `ProblemError` schema enforces this. |
